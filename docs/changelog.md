...
### Fixed
- Chat requests no longer fail on models that reject a parameter the library sends ([#42](https://github.com/pgEdge/pgedge-go-llm-lib/issues/42)). Sonnet 5 and the other Claude models that accept only adaptive thinking refuse a non-default `temperature` with a 400, whilst OpenAI's gpt-6 models refuse `max_tokens` and were never routed to the Responses API, because the OpenAI provider chose both the token parameter and the endpoint from a list of model name prefixes (`o1`, `o3`, `gpt-5`) that every new model generation outgrew. That list is gone: the providers now send the request in its standard form and, when the API returns an error naming a parameter the request carried, adjust it, resend it and apply the same adjustment to every later request from that client, so each costs one extra round trip per client rather than a failure. A request is sent at most three times, so an adjustment that reveals another (moving to `/v1/responses` and then dropping `temperature`, say) still succeeds. The Anthropic provider omits a rejected `temperature`; the OpenAI provider sends `max_completion_tokens` in place of a rejected `max_tokens`, omits a rejected `temperature` on either endpoint, and moves to `/v1/responses` when Chat Completions reports that the model is only available there. A `temperature` rejected alongside extended thinking is dropped for that request alone, since thinking is set per request and may be the cause rather than the model. Errors that do not name a rejected parameter, such as a temperature outside the permitted range, still reach the caller unchanged
...
### Fixed
- `ListModels` and `ListModelsWithMetadata` no longer offer Gemini models that cannot hold a text conversation. Google's `models.list` reports text-to-speech, image generation, music and agent models alongside the conversational ones, and every one of them advertises `generateContent`, which was the only thing the provider filtered on, whilst the response carries no field describing multi-turn support at all. A caller building a model picker from the list therefore presented 36 models where 20 were usable, and a reader who chose one of the other 16 got a failure that reads like a misconfiguration rather than the wrong kind of model: a text-to-speech model refuses the request with `The requested combination of response modalities (TEXT) is not supported by the model ... accepts the following combination of response modalities: * AUDIO`, whilst the agent and research models refuse it with `This model only supports Interactions API`, and `gemini-3.1-flash-tts-preview` manages nothing more helpful than `Request contains an invalid argument`. The families are now excluded by name, as the OpenAI provider has always excluded `tts`, `whisper` and `dall-e`, matching the prefixes `lyria-`, `nano-banana`, `antigravity-` and `deep-research-` and the tokens `-tts` and `-image` anywhere in the identifier, since Gemini qualifies a model's purpose after the generation rather than before it and so needs both forms. Matching on names is unsatisfying and will need occasional maintenance, but no alternative exists: `streamGenerateContent` appears for no model whatsoever, and `createCachedContent`, which does correlate with the conversational models, would wrongly exclude `gemini-omni-flash-preview` and both Gemma models. Deliberately still offered are three families that were suspected and then tested: the robotics models (`gemini-robotics-er-1.6-preview`, `gemini-robotics-er-2-preview`) and the Gemma models both answer an ordinary text prompt, and the computer-use models were never confirmed either way, on the grounds that a model which turns out to be unusable is a smaller problem than one wrongly hidden. There is no opt-out, matching the OpenAI provider's unconditional exclusion; a caller who wants a text-to-speech or image model is not using this library's chat path to reach it. Embedding models are unaffected and continue to be excluded from the default list and available through `WithCapabilities(ModelCapabilityEmbeddings)`

## [0.3.0] - 2026-08-11

### Fixed
- Gemini tool-calling conversations no longer fail on the second turn. Google's current models are thinking models, and they attach an opaque `thoughtSignature` to the part carrying each function call so that they can resume their own reasoning once the tool's result comes back; that value has to be echoed to them verbatim, on the same part, whenever the call is replayed as conversation history, and a Gemini 3 series model refuses the entire request with a 400 rather than merely degrading if it is missing. The provider read neither the signature out of the response nor anything back into the request, so every conversation died the moment a tool result was sent, with `Function call is missing a thought_signature in functionCall parts`. Since almost any useful question needs at least one round of calling a tool and then answering from the result, this made Gemini unusable for tool calling on any key whose available models are current, which is now the ordinary case for a new account rather than an edge case. `llm.ToolUse` gains an opaque `Signature` field, populated by the Gemini provider from both the unary and the streaming response paths and sent back on the function-call part when a tool-use block is converted for a request, and carried across the tool-use accumulators in `Stream.Collect` and the proxy's SSE bridge, which rebuild the block field by field and would otherwise have dropped it. Callers need change nothing so long as they keep assistant messages intact when appending a tool result, which the documented loop already does. Where a response holds parallel function calls only the first part carries a signature, so an empty value on the rest is expected and is not sent, and providers with no such concept leave the field empty
- The Gemini provider now signals tool failures to the model, so a tool that returns an error no longer sends the caller's agentic loop into a retry loop that only ends when it hits its iteration cap. `buildFunctionResponse` built Gemini's `functionResponse` from the tool result's name and text alone and never read `ContentBlock.IsError`, so an errored result arrived looking exactly like a successful one, typically as `{"result": "<error text>"}`, leaving the model with no reason not to reissue the same call; the Anthropic provider has always propagated the flag, which is why the same tools behaved correctly there. A result flagged `IsError` is now reported under an `error` key rather than `result`. Google documents `functionResponse.response` as a free-form JSON object with no protocol-level field for failures, so this is a convention rather than a specification requirement, but it is the one the ecosystem has settled on and it makes the failure legible to the model. The existing parsing behaviour is preserved, so an error whose text is a JSON object is nested under `error` with its structure intact rather than being flattened to a string, and an error carrying no text at all is sent as an explicit placeholder message, since an empty payload would be indistinguishable from a successful call that returned nothing. The success path is byte-for-byte unchanged
- `Chat()` now reports `StopReasonToolUse` for a Gemini response that calls a tool, rather than the generic `StopReasonEndTurn`. Gemini reports `finishReason: "STOP"` for an ordinary function call exactly as it does for a plain text turn, and `parseChatResponse` mapped that straight through with no override, so a caller branching on `response.StopReason == llm.StopReasonToolUse` — the pattern the other three providers all support directly — silently skipped tool execution and saw what looked like an empty, ordinary end of turn. The override is gated strictly on the raw `"STOP"` finish reason rather than on the already-normalised `StopReasonEndTurn`, since Gemini has genuine, documented finish reasons for a tool call gone wrong (`MALFORMED_FUNCTION_CALL`, `UNEXPECTED_TOOL_CALL`) that also fall through `normalizeStopReason`'s default case to that same generic value; gating on the normalised value would have promoted those to `tool_use` too and encouraged a caller to execute a call Gemini had already flagged as broken. `ChatStream` is unaffected: it never set a stop reason on its chunks in the first place, so existing streaming consumers already infer tool use from content themselves

### Changed
- **Source-compatibility note:** the new `Signature` field on `llm.ToolUse` is a fourth field on an exported struct, so an unkeyed composite literal such as `llm.ToolUse{"id", "name", input}` no longer compiles; a keyed literal of the form `llm.ToolUse{ID: "id", Name: "name", Input: input}` is the migration, and is what the documentation and this library's own call sites have always used. Reading the struct, marshalling it, and every keyed literal are unaffected, and the addition is behaviour-preserving for providers that do not populate the field
...
### Security
- Both entries below are covered by advisory [GHSA-8835-xmqj-354h](https://github.com/pgEdge/pgedge-go-llm-lib/security/advisories/GHSA-8835-xmqj-354h), which carries the current identifiers, including a CVE once one is assigned.
...
### Changed
- **Behaviour change:** `Options.WithDefaults()` no longer fills an unset `Temperature` with `0.7`. Previously this made it impossible for a caller to omit `temperature` from the wire: some models (e.g. newer Claude models) reject the field outright with `400: 'temperature' is deprecated for this model`. An unset `Temperature` (on both `Options` and `ChatRequest`) is now genuinely omitted, so a caller who relied on the implicit default will find requests going out without a temperature at all, and should expect the provider's own default to apply instead. An explicitly-set value (including `0`) is unaffected, and setting `Temperature` explicitly restores the previous behaviour exactly
...
- The Ollama provider now honours `Temperature`. `buildChatRequest` never read `req.Temperature` or the client-level `Options.Temperature` at all, so a caller's value was silently dropped regardless of whether it was set, with no error to signal it had no effect — unlike a feature the provider genuinely lacks (Voyage's `Chat`, or a document block here), which returns `ErrNotSupported` instead. It now follows the same per-request → client-default → omit precedence as the other four providers, sent as `options.temperature` on the wire
