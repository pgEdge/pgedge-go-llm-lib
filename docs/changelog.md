...
- `Chat()` now reports `StopReasonToolUse` for a Gemini response that calls a tool, rather than the generic `StopReasonEndTurn`. Gemini has no distinct finish reason for a function call; the API reports `finishReason: "STOP"` whether or not the response carries one, and `parseChatResponse` mapped that straight through with no override, so a caller branching on `response.StopReason == llm.StopReasonToolUse` — the pattern the other three providers all support directly — silently skipped tool execution and saw what looked like an empty, ordinary end of turn. Ollama and OpenAI already derive `StopReasonToolUse` from response content for the same reason (neither has a fully reliable native signal either); Gemini's provider now does the same, overriding the generic end-turn value when `resp.Content` contains a tool-use block. `ChatStream` is unaffected: it never set a stop reason on its chunks in the first place, so existing streaming consumers already infer tool use from content themselves
